...
def search_documents_by_content(keyword, xml_files, xml_texts):
    matched_files = [file for file, text in zip(xml_files, xml_texts) if keyword.lower() in text.lower()]
    return [(file, 0) for file in matched_files]  # スコアは使用しないため、0を設定
...
if st.button("Search"):
    ...
    file_names = [f"{file} - Score: {score:.4f}" for file, score in results]
    selected_filename = st.selectbox("Select an XML file:", file_names)

    # 選択されたファイル名から実際のファイル名を取得
    actual_filename = selected_filename.split(" - Score: ")[0]
    if actual_filename:
        content = get_xml_text_content(os.path.join(xml_directory, actual_filename))
